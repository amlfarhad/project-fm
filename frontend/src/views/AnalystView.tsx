...
export function AnalystView({
  ...
  const provenance = summary?.provenance ?? processResult?.provenance ?? null;
  const sampleStatus = sampleLoaded ? "sample loaded" : "sample ready";
  ...
  return (
    <main className="analyst-shell">
      ...
      <section className="sample-proof-panel" aria-label="Repository sample proof">
        <div className="sample-proof-copy">
          <p className="section-kicker">Primary proof / {sampleStatus}</p>
          <h2>{sample.label}</h2>
          <p>{sample.description} {runtimeMode === "hosted-demo" ? "This hosted view replays the artifact generated by the real pipeline; it does not claim live inference." : "This local view can reprocess the same file through OpenCV."}</p>
          <div className="pipeline-steps" aria-label="Pipeline stages">
            {['input video', 'sampled frames', 'detected players', 'tracked identities', '2D pitch state', 'analyst review'].map((stage, index) => (
              <span key={stage} className={sampleLoaded && index < 5 ? "pipeline-step complete" : index === 5 && sampleLoaded ? "pipeline-step current" : "pipeline-step"}>
                <b>{String(index + 1).padStart(2, "0")}</b>{stage}
              </span>
            ))}
          </div>
          <button className="primary-action sample-action" type="button" onClick={onLoadSample} disabled={processing}>
            <FileVideo size={16} /> {processing ? "Loading pipeline evidence" : sampleLoaded ? "Replay sample again" : "Process repository sample"}
          </button>
        </div>
        <div className="sample-proof-media">
          <video className="source-video" controls preload="metadata" src={sample.video_url} aria-label="Repository sample match clip">
            Your browser cannot play the sample video.
          </video>
          <div className="sample-meta"><span>{sample.license}</span><span>{sample.width} × {sample.height}</span><a href={sample.source_reference} target="_blank" rel="noreferrer">Rights and source ↗</a></div>
        </div>
      </section>
      <section className="analyst-grid">
        ...
        <form className="panel panel-ingest" onSubmit={submitIngest}>
          <div className="panel-heading">
            <h2>File Ingest</h2>
            <span>{runtimeMode === "hosted-demo" ? "hosted proof" : processJob?.status ?? (processing ? "processing" : "ready")}</span>
          </div>
          {runtimeMode === "hosted-demo" && <p className="boundary-note"><ShieldAlert size={15} /> Uploads, streams, and local paths run only from the local Project FM stack.</p>}
          ...
          <button className="primary-action" type="submit" disabled={processing || runtimeMode === "hosted-demo"}>
            <FileVideo size={16} /> {runtimeMode === "hosted-demo" ? "Available locally" : processing ? "Building timeline" : "Process full match"}
          </button>
          ...
          <div className="live-actions">
            {liveStatus === "running" ? (
              ...
              <button
                ...
                type="button"
                disabled={liveStatus === "requesting" || resetting || runtimeMode === "hosted-demo"}
                onClick={startBrowserCapture}
              >
                <ScreenShare size={16} /> {runtimeMode === "hosted-demo" ? "Available locally" : liveStatus === "requesting" ? "Waiting for source" : "Capture browser video"}
              </button>
            )}
          </div>
        ...
        <div className="panel panel-system">
          ...
          <div className="position-legend" aria-label="Position provenance legend">
            <span><i className="legend-dot legend-observed" /> observed</span>
            <span><i className="legend-dot legend-inferred" /> inferred</span>
            <span><i className="legend-dot legend-corrected" /> corrected</span>
            <span><i className="legend-dot legend-unavailable" /> unavailable</span>
          </div>
          {provenance && (
            <div className="provenance-panel">
              <div className="panel-heading"><h3>Evidence and provenance</h3><span>{provenance.execution_mode.replace("_", " ")}</span></div>
              <dl className="metric-list provenance-list">
                <div><dt>Input</dt><dd>{provenance.input_label}</dd></div>
                <div><dt>Source class</dt><dd>{provenance.source_kind.replace("_", " ")}</dd></div>
                <div><dt>Processor</dt><dd>{provenance.processor_backend ?? "not recorded"}</dd></div>
                <div><dt>Pipeline commit</dt><dd>{provenance.pipeline_commit ?? "not recorded"}</dd></div>
                <div><dt>Rights</dt><dd>{provenance.license ?? "operator-supplied"}</dd></div>
              </dl>
              <p className="provenance-note">Observed positions come from detections. Inferred positions come from the reconstruction prior. Corrections are analyst edits; unavailable means the system has no defensible position.</p>
              {provenance.limitations.length > 0 && <ul className="limitation-list">{provenance.limitations.map((limitation) => <li key={limitation}>{limitation}</li>)}</ul>}
            </div>
          )}
            ...
            <tbody>
              {flaggedTracks.map((player) => {
                ...
                <tr key={player.track_id}>
                  ...
                  <td><span className={`position-status position-status-${player.position_status ?? (player.observed ? "observed" : "inferred")}`}>{player.position_status ?? (player.observed ? "observed" : "inferred")}</span></td>
